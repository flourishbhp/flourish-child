...
class TestHivInfantTesting(TestCase):
    ...
    def test_hiv_infant_testing_required_18_months(self):
    ...
    def test_hiv_infant_testing_not_required_older_18_months(self):
        ...
        mommy.make_recipe(
            'flourish_caregiver.maternaldelivery',
            subject_identifier=self.preg_subject_consent.subject_identifier,
            delivery_datetime=get_utcnow() - relativedelta(years=1, months=7),
            live_infants_to_register=1)

        mommy.make_recipe(
            'flourish_child.childbirth',
            subject_identifier=preg_caregiver_child_consent_obj.subject_identifier,
            dob=(get_utcnow() - relativedelta(years=1, months=7)).date(),
            user_created='imosweu')
        ...
        child_consent.dob = (get_utcnow() - relativedelta(days=7)).date()
        child_consent.save_base(raw=True)
